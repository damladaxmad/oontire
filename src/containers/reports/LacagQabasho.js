...
const LacagQabasho = ({name, type}) => {
    ...
    const fetchTransactions = () => {
        setLoading(true)
        axios.get(`${constants.baseUrl}/transactions/get-business-transactions/${business?._id}?customer=notnull&credit=notzero&startDate=${moment(startDate).format("YYYY-MM-DD")}&endDate=${moment(endDate).format("YYYY-MM-DD")}`, {
            headers: {
                "authorization": token
            }
        }).then(res => {
    ...
    const columns = [
        ...
        { title: "Phone", field: "phone", width: "20%", render: (data) => <p>{data?.customer.phone}</p> },
        { title: "Amount", field: "credit", width: "20%", render: (data) => <p>{data?.credit}</p> },
        { title: "Date", field: "date", width: "20%", render: (data) => <p>{moment(data?.date).format("YYYY-MM-DD")}</p> },
 
        { title: "User", field: "user", render: (data) => <p>{data?.user?.username}</p> },
    ]
        ...
        <div style = {{
            ...
            <div>
                <Typography style = {{
                    fontWeight: "bold",
                    fontSize: "20px"
                }}> Lacag Qabasho Report</Typography>
                <Typography style = {{
                    fontSize: "18px",
                    color: "#6C6C6C"
                }}> {loading ? "loading..." : `${transactions?.length} tansactions`}</Typography>
            </div>
            ...
            <div style={{ marginBottom: '20px' }}>
                ...
                onClick={handleViewClick} bgColor="black" />
            </div>  
            ...
            <Typography style = {{
                    fontWeight: "bold",
                    fontSize: "20px"
                }}> ${totalTransactions?.toFixed(2)}</Typography>
            </div>
           
           {name == "customers" && <MyTable columns = {columns} data = {transactions}
            kind = "Qabasho"/>}
